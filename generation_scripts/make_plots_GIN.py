...
class Config:
    ...
    OUT_DIR   = BASE / "gin_embedding_classification"
...
def _fmt_hp_row(r) -> str:
    # concise HP row (no batch size in summary CSV)
...
# Allow extra suffixes after dropout to match your run naming:
# gin_dim{d}_ep{e}_lr{lr}_drop{drop}_pool..._gfeat..._norm..._attr..._ls..._de...
_RUN_RE = re.compile(
    r"^gin_dim(?P<dim>\d+)_ep(?P<ep>\d+)_lr(?P<lr>[^_]+)_drop(?P<drop>[^_]+)(?:_.*)?$"
)
...
def _match_by_folder_name(ds_dir: Path, r: pd.Series) -> Path | None:
    ...
    # exact numeric match on the leading quartet
...
def find_best_run_dir_for_row(r: pd.Series, runs_root: Path) -> Path | None:
    ...
    if p is not None:
        return p
    return _match_by_metrics_json(ds_dir, r)
...
def plot_fp_fn(df_fals, title_prefix, out_dir: Path):
    ...
    plt.bar(x - 0.2, df_fals["FP"], width=0.4, color="#FF6B6B", label="False Positives")
    plt.bar(x + 0.2, df_fals["FN"], width=0.4, color="#4ECDC4", label="False Negatives")
...
def confusion_and_falsifications(df: pd.DataFrame):
    ...
    if Config.PER_DIM_CONFUSION:
        ...
        for ds in sorted(df["dataset"].unique()):
            b = best_per_dim(df, ds, "test_acc")  # fixed: metric was undefined
