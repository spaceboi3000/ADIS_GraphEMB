...
# def load_gin_embeddings_and_labels(run_dir: Path, split: str = "test") -> Tuple[np.ndarray, pd.DataFrame]:
#     """
#     Load embeddings + labels CSV for given run_dir and split (train/val/test).
#     Returns (embeddings, labels_df_with_row_idx)
#     labels_df columns: graph_index, label, row_idx
#     """
...
#     if "graph_index" not in df.columns:
#         raise ValueError(f"'graph_index' column missing in {lab_path}")

#     # row index in embeddings corresponds to row in this CSV
#     df = df.reset_index().rename(columns={"index": "row_idx"})
#     if E.shape[0] != df.shape[0]:
#         raise ValueError(f"Row mismatch: {emb_path} has {E.shape[0]} rows, {lab_path} has {df.shape[0]} rows")
...
#     E_orig, lab_orig = load_gin_embeddings_and_labels(run_orig, split=split)
#     E_perm, lab_perm = load_gin_embeddings_and_labels(run_perm, split=split)

#     # Align by graph_index (inner join)
#     lab_orig = lab_orig[["graph_index", "row_idx", "label"]].rename(
#         columns={"row_idx": "row_orig", "label": "label_orig"}
#     )
#     lab_perm = lab_perm[["graph_index", "row_idx", "label"]].rename(
#         columns={"row_idx": "row_perm", "label": "label_perm"}
#     )

#     merged = lab_orig.merge(lab_perm, on="graph_index", how="inner")

#     if merged.empty:
#         raise ValueError(f"No overlapping graph_index between original and permutated runs for {dataset}/{run_name}")

#     row_orig = merged["row_orig"].to_numpy()
#     row_perm = merged["row_perm"].to_numpy()
