...
def main():
    ...
    # If running from clustering_scripts/, paths should be '../embeddings/...'
    # If running from csv_generation_scripts/, paths should be '../embeddings/...'
    ...
    # Diagnostic: Show resolved paths
    print(f"\n📍 Script directory: {Path(__file__).resolve().parent}")
    print(f"📍 Resolved base_path: {config1.base_path}")
    print(f"📍 Checking if path exists: {Path(config1.base_path).exists()}")
    if Path(config1.base_path).exists():
        print(f"📍 Contents: {list(Path(config1.base_path).iterdir())[:5]}")
    print()
    ...
    config2 = AnalysisConfig(
        ...
        embedding_filename='gin_embeddings',  # Update this after checking actual filename
        random_state=42
    )
