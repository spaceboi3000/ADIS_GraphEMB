...
#                 print(
#                     f"  {dataset} dim={dim} -> X.shape={X.shape}, "
#                     f"n_clusters={n_clusters}, labels={sorted(np.unique(y).tolist())}"
#                 )

#                 metrics = run_kmeans_for_embeddings(X, y, n_clusters)
...
#                         n_graphs=n_samples,
#                         n_clusters=n_clusters,
#                         nmi=round(metrics["nmi"], 6),
#                         ari=round(metrics["ari"], 6),
#                         acc=round(metrics["acc"], 6),
#                         silhouette=round(metrics["silhouette"], 6),
#                     )
#                 )
...
# Base directory where all embeddings live
BASE_EMB_DIR = Path("../permutated_embeddings")
