...
For \textit{unsupervised} methods (Graph2Vec \& NetLSD), graph representations are learned independently of any label information. 
The resulting embeddings are computed once per configuration and stored for subsequent evaluation.
Classification is performed by training standard machine learning models, as we will refer to them in detail in the section \textit{V}, 
...
requirements of each embedding method. Specifically, unsupervised graph embedding methods (Graph2Vec \& NetLSD) 
