...
Early approaches, which based on handcrafted features or graph kernels,  
...
unsupervised~\cite{narayanangraph2vec,tsitsulin2018netlsd} and supervised techniques~\cite{xu2019gin}.
...
In Figure~\ref{fig:SupervsUn} we illustrate the conceptual difference between unsupervised and supervised graph embedding methods.
In the unsupervised setting graph representations are learned solely from the graph structure and 
...
In this work, we focus on the \textbf{EMB1} setting and our purpose is to compare unsupervised and supervised 
graph embedding methods on small benchmark datasets from TUDataset~\cite{morris2020tudataset}.
We evaluate Graph2Vec~\cite{narayanangraph2vec}, NetLSD~\cite{tsitsulin2018netlsd} and GIN~\cite{xu2019gin} under a unified experimental 
