...
G2V_ORIG_ROOT = Path("../embeddings/embeddings_graph2vec") # Graph2Vec paths
G2V_PERM_ROOT = Path("../permutated_embeddings/permutated_graph2vec")
NLSD_ORIG_ROOT = Path("../embeddings/embeddings_netlsd") # NetLSD paths
...
def load_embeddings_pair(orig_path: Path,perm_path: Path,) -> (np.ndarray, np.ndarray):
    ...
    if X_orig.shape != X_perm.shape:
        raise ValueError(f"Shape mismatch: {orig_path} {X_orig.shape} vs {perm_path} {X_perm.shape}")
    
    return X_orig, X_perm


def compute_stability(method: str, dataset: str,dim: int,orig_root: Path, perm_root: Path,) -> Dict:
    ...
    return {"method": method,"dataset": dataset, "dim": dim,"n_graphs": X_orig.shape[0],"cos_mean": float(cos.mean()), "cos_median": float(np.median(cos)),"cos_std": float(cos.std()),"l2_mean": float(l2.mean()),"l2_median": float(np.median(l2)),"l2_std": float(l2.std()), }
...
if __name__ == "__main__":
    ...
    for ds in DATASETS:
        for d in DIMS:
            # Graph2Vec
            try:
                r_g2v = compute_stability( method="Graph2Vec", dataset=ds, dim=d, orig_root=G2V_ORIG_ROOT,perm_root=G2V_PERM_ROOT,)
                rows.append(r_g2v)
            ...
            # NetLSD
            try:
                r_nlsd = compute_stability(method="NetLSD",dataset=ds, dim=d, orig_root=NLSD_ORIG_ROOT, perm_root=NLSD_PERM_ROOT, )
                rows.append(r_nlsd)
