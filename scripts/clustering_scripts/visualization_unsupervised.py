...
BASE_EMB_DIR = Path("../embeddings")  #for Graph2Vec & NetLSD
...
def load_embeddings_csv(method: str,method_cfg: Dict,dataset: str,dim: int,) -> Tuple[np.ndarray, np.ndarray]:
...
def run_kmeans(X: np.ndarray, n_clusters: int) -> np.ndarray:
    km = KMeans(n_clusters=n_clusters,random_state=RANDOM_STATE,n_init=KMEANS_N_INIT,max_iter=KMEANS_MAX_ITER,)
...
def scatter_plot(coords: np.ndarray,labels: np.ndarray,title: str,save_path: Path,legend_title: str,):
    ...
    for lab in unique_labels:
        mask = labels == lab
        plt.scatter(coords[mask, 0],coords[mask, 1],s=12,alpha=0.8,label=str(lab),)
    ...
    for method, cfg in METHODS.items():
        ...
        for dataset in DATASETS:
            ...
            for dim in cfg["dims"]:
                ...
                #true labels
                scatter_plot(X_tsne,y_vis,f"{method} - t-SNE (true labels)",tsne_label_path,"Label",)
                scatter_plot(X_umap,y_vis, f"{method} - UMAP (true labels)", umap_label_path,"Label",)
                # KMeans clusters
                scatter_plot(X_tsne, y_kmeans, f"{method} - t-SNE (KMeans clusters)", tsne_km_path,"Cluster (KMeans)",)
                scatter_plot(X_umap, y_kmeans,f"{method} - UMAP (KMeans clusters)",umap_km_path,"Cluster (KMeans)",)
                #Spectral clusters 
                if y_spectral is not None:
                    scatter_plot( X_tsne, y_spectral,  f"{method} - t-SNE (Spectral clusters)", tsne_sp_path,"Cluster (Spectral)",)
                    scatter_plot(X_umap,  y_spectral, f"{method} - UMAP (Spectral clusters)",umap_sp_path, "Cluster (Spectral)",)
