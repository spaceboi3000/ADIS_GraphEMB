...
def main():
    for dataset in DATASETS:
        ...
        for run_dir in run_dirs:
            ...
            #reall labels
            scatter_plot(X_tsne,y_vis,f"{METHOD_NAME} – {dataset} – {run_dir.name} – t-SNE (labels)",tsne_label_path,"Label",)
            scatter_plot(X_umap,y_vis, f"{METHOD_NAME} – {dataset} – {run_dir.name} – UMAP (labels)", umap_label_path,"Label",)

            #KMeans
            scatter_plot(X_tsne,y_kmeans, f"{METHOD_NAME} – {dataset} – {run_dir.name} – t-SNE (KMeans)", tsne_km_path,"Cluster (KMeans)",)
            scatter_plot( X_umap, y_kmeans,f"{METHOD_NAME} – {dataset} – {run_dir.name} – UMAP (KMeans)", umap_km_path,"Cluster (KMeans)",)

            #spectral
            if y_spectral is not None:
                scatter_plot( X_tsne,y_spectral,f"{METHOD_NAME} – {dataset} – {run_dir.name} – t-SNE (Spectral)",tsne_sp_path,"Cluster (Spectral)",)
                scatter_plot( X_umap, y_spectral,f"{METHOD_NAME} – {dataset} – {run_dir.name} – UMAP (Spectral)", umap_sp_path, "Cluster (Spectral)",)
