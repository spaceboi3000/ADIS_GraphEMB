...
EMBEDDING_DIMS = [64, 128, 256]    #dimss
...
def make_graph2vec_params(dim: int) -> Dict:
    return dict(dimensions=dim, wl_iterations=2,min_count=5,learning_rate=0.025, epochs=15, seed=42,)
...
def run_graph2vec_for_dataset(name: str):
    ...
    for dim in EMBEDDING_DIMS:
        ...
        metrics_rows.append({  "dataset": name,"method": "Graph2Vec", "dim": dim, "n_graphs": len(graphs),"fit_time_s": round(fit_time, 4), "embed_time_s": round(embed_time, 4),"total_time_s": round(total_time, 4),"rss_before_mb": round(rss_before_mb, 2), "rss_after_mb": round(rss_after_mb, 2),"peak_tracemalloc_mb": round(peak_mb, 2),})
