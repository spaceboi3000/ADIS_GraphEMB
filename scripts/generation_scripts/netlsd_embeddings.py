...
NETLSD_PARAMS: Dict = dict(scale_min=None,scale_max=None, scale_n=None)
...
def _assert_local_available(root: Path, name: str):
    ...
    if not any(p.exists() for p in _tu_processed_paths(root, name)):
        raise FileNotFoundError( f"Local TU dataset not found for '{name}'. Checked: " + ", ".join(map(str, _tu_processed_paths(root, name))))
...
def run_netlsd_for_dataset(name: str):
    ...
    for dim in TARGET_DIMS:
        ...
        metrics_rows.append({"dataset": name, "method": "NetLSD", "dim": dim,"n_graphs": len(graphs),"fit_time_s": round(fit_time, 4), "pca_time_s": round(pca_time, 4), "total_time_s": round(total_time, 4),"rss_before_mb": round(rss_before_mb, 2), "rss_after_mb": round(rss_after_mb, 2),"peak_tracemalloc_mb": round(peak_mb, 2),"raw_sig_len": X_raw.shape[1], })
