...
CANDIDATES = [
    HERE / "embeddings_gin_version2" / "plots_GIN" / "best_gin_summary.csv",
    HERE / "embeddings_gin_version2" / "best_gin_summary.csv",
...
def normalize_columns(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # sometimes columns come as strings; coerce what we need
    to_numeric = ["dim","epochs","lr","val_acc","test_acc","f1_macro","auc_macro","total_time_s","peak_tracemalloc_mb"]
    for c in to_numeric:
        if c in df.columns:
...
def load_summary(csv_path: Path) -> pd.DataFrame:
    ...
    # ALWAYS create a label (no batch size as requested)
    df["label"] = df["dataset"]
...
def plot_hparams_alt(df: pd.DataFrame, out_dir: Path):
    ...
    ax = axes[2]
    # Replace non-positive/NaN with tiny positive so log scale renders;
    # but annotate the TRUE textual value with fmt_sci.
    lr_safe = df["lr"].copy()
